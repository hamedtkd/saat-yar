...
const privacyFa: LegalPageCopy = {
  ...
  sections: [
    ...
    { title: "آمار ناشناس محصول", paragraphs: ["در نسخه‌هایی که Google Analytics 4 پیکربندی شده، ساعت‌یار آمار ناشناس و کلی محصول را به‌صورت پیش‌فرض فعال می‌کند؛ مانند بازدید یک بخش، تکمیل onboarding، شروع/پایان کار یا استفاده از یک قابلیت. کاربر می‌تواند هر زمان از Settings این آمار را خاموش کند.", "نام، یادداشت، عنوان مشتری یا پروژه، مبلغ حقوق یا درآمد، متن آزاد، شناسه رکورد، تاریخ کاری و ساعت دقیق در taxonomy تحلیل محصول ارسال نمی‌شوند. تبلیغات، ad personalization و سیگنال‌های تبلیغاتی برای این integration غیرفعال‌اند.", "در نواحی‌ای که رضایت برای analytics storage لازم است، Google Consent Mode ذخیره‌سازی تحلیلی را تا زمان اجازه صریح روی حالت denied نگه می‌دارد؛ خاموش‌کردن آمار از Settings ارسال را متوقف می‌کند."] },
...
const privacyEn: LegalPageCopy = {
  ...
  sections: [
    ...
    { title: "Anonymous product analytics", paragraphs: ["When Google Analytics 4 is configured, Saatyar enables coarse anonymous product analytics by default, such as route views, onboarding completion, work start/finish, or feature use. You can turn analytics off at any time from Settings.", "Saatyar's analytics taxonomy excludes names, notes, client/project titles, salary or income amounts, free text, record IDs, work dates, and exact clock times. Advertising signals and ad personalization are disabled for this integration.", "In regions where analytics-storage consent is required, Google Consent Mode keeps analytics storage denied until an explicit grant. Turning analytics off in Settings stops analytics delivery."] },
...
const termsFa: LegalPageCopy = {
  ...
  sections: [
    ...
    { title: "سرویس‌های خارجی", paragraphs: ["قابلیت‌هایی مانند Google Calendar و Google Analytics تابع شرایط و سیاست‌های سرویس ارائه‌دهنده نیز هستند. Google Calendar فقط بعد از اقدام کاربر متصل می‌شود؛ آمار ناشناس GA4 در build پیکربندی‌شده به‌صورت پیش‌فرض فعال است و از Settings قابل خاموش‌کردن است.", "کاربر می‌تواند Google Calendar را Disconnect کند و تحلیل اختیاری محصول را از Settings خاموش کند."] },
...
const termsEn: LegalPageCopy = {
  ...
  sections: [
    ...
    { title: "External services", paragraphs: ["External services such as Google Calendar and Google Analytics are also subject to the relevant provider's terms and policies. Google Calendar connects only after a user action; configured GA4 anonymous product analytics is enabled by default and can be turned off in Settings.", "You can disconnect Google Calendar and turn optional product analytics off in Settings."] },
