...
export function exportExcel(filename: string, title: string, headers: string[], rows: unknown[][]) {
  const tableRows = [headers, ...rows]
    .map((row, index) => `<tr>${row.map((cell) => `<${index === 0 ? "th" : "td"}>${String(cell ?? "")}</${index === 0 ? "th" : "td"}>`).join("")}</tr>`)
    .join("");
  const html = `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>${title}</title></head><body><table>${tableRows}</table></body></html>`;
