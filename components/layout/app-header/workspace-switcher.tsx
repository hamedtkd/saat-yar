...
export function WorkspaceSwitcher({ mode, onChange }: { mode: Mode; onChange: (mode: Mode) => void }) {
  return <div className="flex h-10 items-center gap-1.5 rounded-[var(--control-radius)] border border-[var(--border)] bg-[var(--surface-1)] px-1.5 max-[640px]:hidden">
    ...
    <Select value={mode} onValueChange={(value) => onChange(value as Mode)}>
      <SelectTrigger aria-label="تغییر سریع فضای کاری" className="h-8 min-w-[96px] border-0 bg-transparent px-1.5 text-xs font-extrabold shadow-none focus:ring-0 data-[state=open]:ring-0"><SelectValue /></SelectTrigger>
