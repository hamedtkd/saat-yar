...
export function WorkspaceSwitcher({ mode, onChange }: { mode: Mode; onChange: (mode: Mode) => void }) {
  return <div className="flex h-11 items-center gap-2 rounded-[var(--control-radius)] border border-[var(--border)] bg-[var(--surface-2)] px-2 max-[640px]:hidden">
    ...
    <Select value={mode} onValueChange={(value) => onChange(value as Mode)}>
      <SelectTrigger aria-label="تغییر سریع فضای کاری" className="h-8 min-w-[104px] border-0 bg-transparent text-xs font-extrabold shadow-none"><SelectValue /></SelectTrigger>
