    ...
    <div className="flex min-w-0 items-center justify-end gap-1.5 sm:gap-2">
      <span
        className={
          props.saveState === "error"
            ? "hidden text-xs font-bold text-[var(--danger)] 2xl:inline-flex"
            : "hidden items-center gap-1.5 text-xs font-bold text-[var(--accent-strong)] 2xl:inline-flex"
        }
        role="status"
        aria-live="polite"
      >
      ...
      <div className="flex items-center gap-1 rounded-[var(--control-radius)] border border-[var(--border)] bg-[var(--surface-2)] p-1">
        ...
        <Button
          ...
          aria-label={props.financialsHidden ? "نمایش اطلاعات مالی" : "مخفی کردن اطلاعات مالی"}
        >
          {props.financialsHidden ? <EyeOff aria-hidden="true" /> : <Eye aria-hidden="true" />}
        </Button>
        <Button className="size-9 border-0 bg-transparent shadow-none max-[760px]:hidden" variant="outline" size="icon" onClick={props.onExport} aria-label="دانلود پشتیبان">
          <Download aria-hidden="true" />
        </Button>
        <Button className="size-9 border-0 bg-transparent shadow-none max-[980px]:hidden" variant="outline" size="icon" onClick={props.onSettings} aria-label="باز کردن تنظیمات">
