...
const SelectTrigger = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Trigger>>(({ className, children, ...props }, ref) => {
  ...
  return (
    <SelectPrimitive.Trigger dir={direction} ref={ref} className={cn(
      "group relative flex h-11 w-full min-w-0 items-center justify-start gap-2 overflow-hidden rounded-[var(--control-radius)] border border-[var(--border)] bg-[var(--surface-2)] py-2 ps-3 pe-10 text-start text-xs font-medium text-[var(--text)] outline-none transition-[border-color,box-shadow,background-color]",
      ...
      className,
    )} {...props}>{children}<SelectPrimitive.Icon asChild><ChevronDown aria-hidden="true" className="pointer-events-none absolute end-3 size-4 shrink-0 text-[var(--text-muted)] transition-transform group-data-[state=open]:rotate-180" /></SelectPrimitive.Icon></SelectPrimitive.Trigger>
  );
});
...
const SelectContent = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Content>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Content>>(({ className, children, position = "popper", sideOffset = 7, align = "start", collisionPadding = 12, ...props }, ref) => {
  const { direction } = useLocale();
  return (
    <SelectPrimitive.Portal><SelectPrimitive.Content ref={ref} dir={direction} position={position} sideOffset={sideOffset} align={align} collisionPadding={collisionPadding} className={cn(
...
const SelectItem = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Item>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item>>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item className={cn(
    "relative flex min-h-10 w-full cursor-default select-none items-center rounded-lg py-2 ps-9 pe-3 text-start text-xs outline-none transition-colors focus:bg-[var(--accent-soft)] focus:text-[var(--accent-strong)] data-[highlighted]:bg-[var(--accent-soft)] data-[highlighted]:text-[var(--accent-strong)] data-[state=checked]:bg-[var(--accent-soft)] data-[state=checked]:font-bold data-[state=checked]:text-[var(--accent-strong)] data-[disabled]:pointer-events-none data-[disabled]:opacity-45",
    className,
  )} ref={ref} {...props}><span className="absolute start-3 flex size-4 items-center justify-center text-[var(--accent-strong)]"><SelectPrimitive.ItemIndicator><Check className="size-4 stroke-[2.4]" /></SelectPrimitive.ItemIndicator></span><SelectPrimitive.ItemText>{children}</SelectPrimitive.ItemText></SelectPrimitive.Item>
));
