    ...
    <section className="grid grid-cols-3 gap-4 max-[1180px]:grid-cols-2 max-[620px]:grid-cols-1">
      {data.projects.map((project) => {
        ...
        return <SurfaceCard as="article" className="group cursor-pointer p-5 hover:-translate-y-0.5 hover:border-[color-mix(in_srgb,var(--accent)_32%,var(--dashboard-border))] hover:shadow-[0_14px_34px_rgba(0,0,0,.07)]" key={project.id} onClick={() => onSelect(project.id)}>
          ...
          <div className="my-5 grid grid-cols-2 gap-3"><div className="rounded-[var(--control-radius)] border border-[var(--dashboard-border)] bg-[var(--surface-2)] p-3 text-xs text-[var(--text-muted)]"><span>زمان ثبت‌شده</span><strong className="mt-1 block text-lg text-[var(--text)]">{duration(summary.trackedMinutes)}</strong></div><div className="rounded-[var(--control-radius)] border border-[var(--dashboard-border)] bg-[var(--surface-2)] p-3 text-xs text-[var(--text-muted)]"><span>سود خالص</span><strong className="mt-1 block text-lg text-[var(--text)]"><PrivateMoney value={summary.profit} hidden={financialsHidden} /></strong></div></div>
