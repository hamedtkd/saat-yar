    ...
    <SurfaceCard as="section" className="mb-5 flex min-h-[118px] items-center justify-between gap-6 p-5 max-[720px]:flex-col max-[720px]:items-stretch">
      <div className="min-w-0">
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <div className="min-w-0"><h1 className="truncate text-[clamp(1.35rem,2.4vw,2rem)] font-black text-[var(--text)]">{project.name}</h1><p className="mt-1 text-xs text-[var(--text-muted)]">{b("projects.detail.client", { client: client?.name ?? "—" })}</p></div>
        </div>
      </div>
      <div className="flex items-center gap-2 max-[720px]:w-full max-[720px]:flex-wrap">
        <Button onClick={onToggleTimer}>{isRunning ? <><Square /> {b("projects.detail.timerStop")}</> : <><Play /> {b("projects.detail.timerStart")}</>}</Button>
        <Button variant="outline" onClick={onToggleStatus}><Pause /> {project.status === "active" ? b("projects.detail.pause") : b("projects.detail.activate")}</Button>
      </div>
    </SurfaceCard>
  );
}
