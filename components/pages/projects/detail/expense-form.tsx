...
export function ExpenseForm({ draft, setDraft, onSave, onCancel }: {
  ...
  return (
    <div className="mb-4 grid grid-cols-4 gap-3 rounded-[var(--control-radius)] bg-[var(--surface-2)] p-3 max-[900px]:grid-cols-2 max-[620px]:grid-cols-1">
      ...
      <div className="grid gap-1 text-xs"><span className="font-semibold text-[var(--text-muted)]">تاریخ</span><JalaliDatePicker value={draft.date} onChange={(date) => setDraft((value) => ({ ...value, date }))} placeholder="تاریخ هزینه" /></div>
