...
export function EmployeeReportTable({ monthRecords, settings, financialsHidden }: Props) {
  ...
  return <article className={cn("min-w-0 overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface-glass)] shadow-[0_10px_35px_rgba(17,45,55,0.055)]")}>
    ...
    {monthRecords.length > 0 ? <><EmployeeDesktopTable monthRecords={monthRecords} settings={settings} dailyTarget={dailyTarget} totals={totals} financialsHidden={financialsHidden} /><EmployeeMobileCards monthRecords={monthRecords} settings={settings} dailyTarget={dailyTarget} totals={totals} financialsHidden={financialsHidden} /></> : <div className="p-4 sm:p-5"><EmptyState icon={<Filter />} title={t("reports.table.employeeEmpty")} description={t("reports.table.employeeEmptyHint")} /></div>}
  </article>;
}
