...
export function TodayPage(props: TodayPageProps) {
  return (
    <>
      ...
      <TodayMetrics data={props.data} record={props.record} selectedDate={props.selectedDate} result={props.todayCalc} dailyTarget={props.dailyTarget} />
      <TodayTimeline {...props} />
      {props.record.start && !props.record.end && props.todayCalc.worked > 4 * 60 && <div className={cn("mt-[14px] flex items-center gap-[13px] rounded-[14px] border border-[#f1c36c] bg-[#fff9eb] px-[19px] py-[14px] text-[#8b5a05] [&>svg]:h-[27px] [&>svg]:w-[27px] [&>div]:grid [&>div]:flex-1 [&_span]:text-[10px] [&_span]:text-[#78694c] print:hidden")}><AlertTriangle /><div><strong>بیش از ۴ ساعت از شروع روز گذشته است.</strong><span>برای حفظ دقت ثبت زمان، یک استراحت کوتاه یا بررسی تایمر پیشنهاد می‌شود.</span></div><Button variant="outline" onClick={props.startBreak}>شروع وقفه</Button></div>}
    </>
  );
}
