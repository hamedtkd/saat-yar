...
export function TodayFocusCard(props: Props) {
  ...
  return (
    <SurfaceCard className="mb-5 overflow-hidden">
      <div className={cn("grid grid-cols-[minmax(0,1fr)_320px] max-[980px]:grid-cols-1", !isEmployee && "grid-cols-[minmax(0,1fr)_340px]")}>
        <div className={cn("p-5 sm:p-6", isEmployee ? "flex min-h-52 items-center" : "grid grid-cols-12 content-center gap-4")}>
          {!isEmployee && <>
          ...
          <label className={cn("grid min-w-0 gap-2 text-xs font-bold text-[var(--text-muted)]", isEmployee ? "w-full" : "col-span-8 max-[720px]:col-span-12")}>توضیحات<Input placeholder="شرح کوتاه کار امروز" value={props.timerDraft.note} onChange={(event) => props.setTimerDraft((previous) => ({ ...previous, note: event.target.value }))} /></label>
