...
export function TodayTimeline(props: Pick<TodayPageProps, "data" | "selectedDate" | "editingEntry" | "setEditingEntry" | "setData" | "toggleProjectTimer" | "setTab">) {
  ...
  return (
    <section className={cn("grid grid-cols-[minmax(0,1fr)_320px] gap-[14px] max-[900px]:grid-cols-1")}>
      <article className={cn("rounded-[15px] border border-[#dfe7e9] bg-white/95 shadow-[0_10px_35px_rgba(17,45,55,.055)] p-4", "min-w-0 p-[13px]")}>
        ...
        <div className={cn("flex justify-center px-0 pb-0 pt-3 text-[11px] text-[#6c7d89] [&_strong]:text-base [&_strong]:text-[#102a3a]")}><span>جمع کل: <strong>{duration(entries.reduce((sum, entry) => sum + entryMinutes(entry), 0))}</strong></span></div>
      </article>
      {props.data.settings.mode !== "employee" && <aside className={cn("rounded-[15px] border border-[#dfe7e9] bg-white/95 shadow-[0_10px_35px_rgba(17,45,55,.055)] p-4", "p-[17px] max-[900px]:order-first [&>button]:mt-3")}><PanelHead icon={<Info />} title="پروژه‌های اخیر" />{recentProjects.map((project) => <div className={cn("grid grid-cols-[9px_1fr_auto] items-center gap-[10px] border-b border-[#edf1f2] py-3 [&>span]:h-[9px] [&>span]:w-[9px] [&>span]:rounded-full [&>div]:grid [&_strong]:text-xs [&_small]:text-[9px] [&_small]:text-[#6c7d89]")} key={project.id}><span style={{ background: project.color }} /><div><strong>{project.name}</strong><small>{props.data.clients.find((item) => item.id === project.clientId)?.name}</small></div><Button variant="outline" size="sm" onClick={() => props.toggleProjectTimer(project.id)}><Play /> شروع</Button></div>)}{recentProjects.length === 0 && <EmptyState compact icon={<Folder />} description="از صفحه پروژه‌ها اولین پروژه را بساز." />}<Button variant="ghost" className={cn("w-full")} onClick={() => props.setTab("projects")}>مشاهده همه پروژه‌ها</Button></aside>}
    </section>
  );
}
