...
export function ManualEntryForm(props: Pick<TodayPageProps, "data" | "selectedDate" | "setData" | "setEditingEntry" | "createClient" | "createProject">) {
  ...
  function save(event: React.FormEvent) {
    ...
    if (!project) return setError("برای ثبت دستی، ابتدا یک پروژه را انتخاب یا همین‌جا ایجاد کن.");
    ...
    if (!Number.isFinite(startedAt) || !Number.isFinite(endedAt) || endedAt <= startedAt) return setError("زمان پایان باید بعد از زمان شروع باشد.");
    ...
    if (overlaps) return setError("این بازه با یک رکورد زمانی دیگر هم‌پوشانی دارد؛ ساعت‌ها را اصلاح کن.");
  ...
  return (
    <SurfaceCard className="mb-[18px] p-5 [&_.panel-head]:mb-4">
      <PanelHead icon={<Plus />} title="ثبت دستی زمان پروژه"><Button type="button" variant="ghost" onClick={() => props.setEditingEntry("")}>بستن</Button></PanelHead>
      <form onSubmit={save} noValidate>
        ...
        <div className="mb-4 grid grid-cols-3 gap-3.5 max-[760px]:grid-cols-2 max-[620px]:grid-cols-1">
          <div className="grid gap-2 text-xs font-semibold text-[var(--text-muted)]">
            ...
            {activeClients.length ? (
              ...
              <div className="flex min-h-11 items-center gap-2 rounded-[var(--control-radius)] border border-dashed border-[var(--dashboard-border)] bg-[var(--surface-2)] px-3 text-[10px]"><Users aria-hidden="true" className="size-4" /><span>هنوز مشتری نداری؛ مشتری را همین‌جا بساز.</span></div>
            )}
          </div>
          <div className="grid gap-2 text-xs font-semibold text-[var(--text-muted)]">
            ...
            {!selectedClientId ? (
              <div className="flex min-h-11 items-center gap-2 rounded-[var(--control-radius)] border border-dashed border-[var(--dashboard-border)] bg-[var(--surface-2)] px-3 text-[10px]"><FolderPlus aria-hidden="true" className="size-4" /><span>ابتدا مشتری را انتخاب کن.</span></div>
            ) : projects.length ? (
              ...
              <div className="flex min-h-11 items-center gap-2 rounded-[var(--control-radius)] border border-dashed border-[var(--dashboard-border)] bg-[var(--surface-2)] px-3 text-[10px]"><FolderPlus aria-hidden="true" className="size-4" /><span>برای این مشتری پروژه‌ای نیست؛ «پروژه جدید» را بزن.</span></div>
            )}
          </div>
          <label className="grid gap-2 text-xs font-semibold text-[var(--text-muted)]">شروع<TimePicker value={draft.start} onChange={(start) => { setError(""); setDraft({ ...draft, start }); }} /></label>
          <label className="grid gap-2 text-xs font-semibold text-[var(--text-muted)]">پایان<TimePicker value={draft.end} onChange={(end) => { setError(""); setDraft({ ...draft, end }); }} /></label>
          <label className="col-span-2 grid gap-2 text-xs font-semibold text-[var(--text-muted)] max-[620px]:col-auto">شرح<Input autoFocus value={draft.note} onChange={(event) => setDraft({ ...draft, note: event.target.value })} placeholder="مثلاً جلسه طراحی" /></label>
        ...
        <div className="flex flex-wrap items-center gap-2"><Button type="submit"><Save /> ذخیره ورودی</Button><span className="text-[10px] text-[var(--text-muted)]">Enter ذخیره می‌کند و خطا بدون پنجره مزاحم در همین فرم نمایش داده می‌شود.</span></div>
      </form>
    </SurfaceCard>
  );
}
