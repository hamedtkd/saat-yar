...
export function InvoiceForm({ clients, projects, draft, setDraft, createClient, createProject, onSave, onCancel }: {
  ...
  const errors = submitted ? validateInvoiceDraft(draft) : {};
  ...
  const submit = (event: React.FormEvent) => {
    ...
    const nextErrors = validateInvoiceDraft(draft);
    setSubmitted(true);
    ...
    <SurfaceCard as="section" className="mb-5 p-5">
      ...
      <form onSubmit={submit} noValidate>
        ...
        <div className="grid grid-cols-4 gap-3 max-[900px]:grid-cols-2 max-[620px]:grid-cols-1">
          ...
          <div className={fieldClass}><span>تاریخ صدور</span><JalaliDatePicker value={draft.issuedAt} onChange={(issuedAt) => setDraft((value) => ({ ...value, issuedAt }))} placeholder="تاریخ صدور" /><FieldError message={errors.issuedAt} /></div>
          <div className={fieldClass}><span>تاریخ سررسید</span><JalaliDatePicker value={draft.dueAt} onChange={(dueAt) => setDraft((value) => ({ ...value, dueAt }))} placeholder="تاریخ سررسید" /><FieldError message={errors.dueAt} /></div>
          <label className={`${fieldClass} col-span-2 max-[620px]:col-auto`}>شرح<Input autoFocus aria-invalid={Boolean(errors.description)} value={draft.description} onChange={(event) => setDraft((value) => ({ ...value, description: event.target.value }))} /><FieldError message={errors.description} /></label>
          <label className={fieldClass}>تعداد<NumberField value={draft.quantity} onValueChange={(quantity) => setDraft((value) => ({ ...value, quantity }))} /><FieldError message={errors.quantity} /></label>
          <label className={fieldClass}>مبلغ واحد<NumberField value={draft.unitPrice} onValueChange={(unitPrice) => setDraft((value) => ({ ...value, unitPrice }))} /><FieldError message={errors.unitPrice} /></label>
          <label className={fieldClass}>تخفیف<NumberField value={draft.discount} onValueChange={(discount) => setDraft((value) => ({ ...value, discount }))} /><FieldError message={errors.discount} /></label>
          <label className={fieldClass}>مالیات (درصد)<NumberField value={draft.taxPercent} onValueChange={(taxPercent) => setDraft((value) => ({ ...value, taxPercent }))} /><FieldError message={errors.taxPercent} /></label>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2"><Button type="submit">ذخیره فاکتور</Button><Button type="button" variant="outline" onClick={onCancel}>لغو</Button><span className="text-[10px] text-[var(--text-muted)]">با Enter ذخیره کن؛ خطاها قبل از ثبت کنار همان فیلد دیده می‌شوند.</span></div>
      </form>
    </SurfaceCard>
  );
}
