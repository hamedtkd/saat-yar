...
export function PayrollPolicyPreview({ preview, hidden }: { preview: ReturnTypeOfPayrollPreview; hidden: boolean }) {
  ...
  const basisText = preview.rateSummary.basis === "standard-month"
    ? `${s("Standard month (recommended)")} · ${number(preview.rateSummary.standardMonthMinutes / 60)} ${s("Hours")}`
    : s("Period target hours");
  return (
    <aside className="grid gap-3 rounded-[var(--card-radius)] border border-[var(--dashboard-border)] bg-[var(--surface-2)] p-4" data-payroll-rate-preview>
      ...
      <div className="grid grid-cols-3 gap-2 max-[900px]:grid-cols-2 max-[520px]:grid-cols-1">
        <Mini label={s("Worked")} value={duration(preview.facts.workedMinutes)} />
        <Mini label={s("Target")} value={duration(preview.facts.targetMinutes)} />
...
function MiniMoney({ label, value, hidden }: { label: string; value: number; hidden: boolean }) { const { s } = useSystemUi(); return <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-1)] p-2.5"><small className="block text-[9px] text-[var(--text-muted)]">{label}</small><strong className="mt-1 block text-xs text-[var(--text)]"><PrivateMoney value={value} hidden={hidden} /> {s("Toman")}</strong></div>; }
