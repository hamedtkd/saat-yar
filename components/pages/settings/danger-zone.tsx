...
export function DangerZone({ setData, setToast }: { setData: React.Dispatch<React.SetStateAction<AppData>>; setToast: (message: string) => void }) {
  return (
    <section className={cn("rounded-[15px] border border-[var(--border)] bg-[var(--surface-1)] shadow-[0_10px_35px_rgba(17,45,55,.055)] p-4", "col-span-full flex items-center justify-between gap-5 border-[color-mix(in_srgb,var(--danger)_35%,var(--border))] bg-[var(--danger-soft)] max-[620px]:col-auto max-[620px]:items-stretch max-[620px]:flex-col [&>div]:flex [&>div]:items-center [&>div]:gap-[11px] [&>div>svg]:h-7 [&>div>svg]:w-7 [&>div>svg]:text-[var(--danger)] [&>div>div]:grid [&_span]:text-[10px] [&_span]:text-[var(--text-muted)]")}>
