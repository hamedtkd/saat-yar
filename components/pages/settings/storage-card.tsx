...
export function StorageCard({ storage, requestPersistence }: { storage: StorageInfo; requestPersistence: () => Promise<void> }) {
  ...
  const usageMb = number(storage.usage / 1024 / 1024, { maximumFractionDigits: 1 });
  const quotaGb = number(storage.quota / 1024 / 1024 / 1024, { maximumFractionDigits: 1 });
  return (
    <section id="settings-storage" className={cn("scroll-mt-24 dashboard-card rounded-[var(--card-radius)] border border-[var(--dashboard-border)] shadow-[0_5px_16px_rgba(0,0,0,.03)] p-4", "p-5")}>
      <PanelHead icon={<HardDrive />} title={s("Storage")} />
      <dl className={cn("m-0 [&>div]:flex [&>div]:justify-between [&>div]:gap-[10px] [&>div]:py-2 [&_dt]:text-[10px] [&_dt]:text-[var(--text-muted)] [&_dd]:m-0 [&_dd]:text-[11px]")}>
      ...
      <Button className={cn("w-full")} onClick={() => void requestPersistence()}><ShieldCheck /> {storage.persisted ? s("Persistent storage is enabled") : s("Request persistent storage")}</Button>
      <p className={cn("mt-3 flex items-start gap-[7px] text-[10px] leading-[1.8] text-[var(--text-muted)] [&_svg]:mt-0.5 [&_svg]:w-[14px] [&_svg]:flex-none")}><Info />{s("Your OS or browser may clear local data when storage is low. Regular backups are recommended.")}</p>
    </section>
  );
}
