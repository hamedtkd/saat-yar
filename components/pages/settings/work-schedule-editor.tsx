...
import { applyLunchMinutesToAll, applyLunchPaidToAll, applyScheduleDayToEnabledDays, applyWeeklyTargetHours, getScheduleTargetMinutes, getWeeklyTargetMinutes, updateScheduleLunch, weekdayOrder } from "@/lib/work-schedule";
...
export function WorkScheduleEditor({ value, disabled, onChange }: { value: WorkSettingsDraft; disabled: boolean; onChange: (next: WorkSettingsDraft) => void }) {
  ...
  return (
    <section data-work-schedule-editor className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface-2)]">
      ...
      <div className="grid gap-2 p-3 sm:p-4">{weekdayOrder.map((day) => { const schedule = value.weeklySchedule[day]; return <div key={day} data-work-schedule-day={day} className="grid grid-cols-[150px_repeat(3,minmax(130px,1fr))_128px] items-end gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-1)] p-3 max-[980px]:grid-cols-2 max-[620px]:grid-cols-1"><div className="grid gap-1"><label className="flex min-h-9 cursor-pointer items-center gap-2 rounded-lg px-1"><Checkbox data-workday-toggle={day} disabled={disabled} checked={schedule.enabled} onCheckedChange={(enabled) => setDay(day, "enabled", enabled)} /><strong className={schedule.enabled ? "text-[var(--text)]" : "text-[var(--text-muted)]"}>{s(weekdayKeys[day])}</strong></label>{schedule.enabled && <Button type="button" size="sm" variant="ghost" disabled={disabled} data-apply-schedule-source={day} className="h-7 justify-start px-1 text-[9px] text-[var(--text-muted)]" title={s("Copies start, end, lunch duration, and lunch paid state. Disabled days stay unchanged.")} onClick={() => onChange(applyScheduleDayToEnabledDays(value, day))}><Copy aria-hidden="true" className="size-3" /> {s("Apply to active days")}</Button>}</div><label>{s("Start")}<TimePicker disabled={disabled} value={schedule.start} onChange={(next) => setDay(day, "start", next)} /></label><label>{s("End")}<TimePicker disabled={disabled} value={schedule.end} onChange={(next) => setDay(day, "end", next)} /></label><label>{s("Lunch")}<MinuteDurationField disabled={disabled} value={schedule.lunchMinutes} onValueChange={(next) => setDayLunch(day, next)} /></label><div className="grid min-h-11 place-items-center rounded-lg bg-[var(--surface-2)] px-2 py-1 text-center text-[10px] font-bold leading-4 text-[var(--text-muted)]">{schedule.enabled ? <><strong className="text-[var(--text)]">{durationWords(getScheduleTargetMinutes(schedule))}</strong><small className="text-[9px] font-medium">{s("Net work for the day")}</small></> : s("Day off")}</div></div>; })}</div>
    </section>
  );
}
