  ...
  return (
    <fieldset disabled={disabled} className="grid gap-4 disabled:opacity-70">
      <div className="grid grid-cols-3 gap-3 max-[900px]:grid-cols-2 max-[620px]:grid-cols-1">
        ...
        <label>{s("Standard hours per day")}<NumberField step="0.25" value={policy.standardDayMinutes / 60} min={0.25} onValueChange={(hours) => patch({ standardDayMinutes: Math.max(15, Math.round(hours * 60)) })} /></label>
      </div>
