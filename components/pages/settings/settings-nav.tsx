...
export function SettingsNav() {
  ...
  return (
    <aside aria-label="بخش‌های تنظیمات" className="sticky top-[84px] z-20 self-start max-[900px]:top-[72px]">
      <div className="dashboard-card max-h-[calc(100dvh-104px)] overflow-y-auto rounded-[var(--card-radius)] border border-[var(--dashboard-border)] p-2.5 shadow-[0_6px_20px_rgba(0,0,0,.035)] max-[900px]:hidden">
        {settingsNavGroups.map((group) => {
          ...
          const activeLabel = isActiveGroup ? settingsNavItems.find((item) => item.id === active)?.label : null;
          return (
            <div key={group.id} className="border-b border-[var(--border)] py-1.5 last:border-b-0">
              <button
                ...
                className={cn(
                  "flex min-h-10 w-full items-center gap-2 rounded-xl px-2.5 text-right transition-colors",
              ...
              >
                <span className="min-w-0 flex-1">
                  <strong className="block text-[10px] font-black">{group.label}</strong>
              ...
              {isOpen && (
                <div className="mt-1 grid gap-0.5 pr-2">
                  {items.map(({ id, label, icon: Icon }) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "group flex min-h-9 w-full items-center gap-2.5 rounded-[11px] px-2.5 text-right text-[10px] font-bold text-[var(--text-muted)] transition-colors",
                      ...
                      >
                        ...
                        {isActive && <span aria-hidden="true" className="mr-auto size-1.5 rounded-full bg-[var(--accent)]" />}
                      </button>
                    );
                  })}
                </div>
      ...
      <div className="hidden gap-1.5 rounded-[18px] border border-[var(--dashboard-border)] bg-[var(--surface-glass)] p-1.5 shadow-[0_6px_20px_rgba(0,0,0,.035)] backdrop-blur-xl max-[900px]:grid">
        ...
        <div className="flex gap-1 overflow-x-auto">
          {getSettingsGroupItems(activeGroup).map(({ id, label, icon: Icon }) => {
            const isActive = active === id;
            return (
              <button
                key={id}
                ...
                onClick={() => requestNavigation(() => navigateTo(id as SettingsItemId))}
                className={cn(
                  "flex min-h-9 min-w-max shrink-0 items-center gap-2 rounded-xl px-3 text-[9px] font-bold text-[var(--text-muted)] transition-colors",
                  "hover:bg-[var(--surface-2)] hover:text-[var(--text)]",
