...
import { BellRing, Coffee, PauseCircle, PlayCircle, Send, TimerOff } from "lucide-react";
...
import { breakReminderSnoozeKey, notificationSnoozeKey, parseNotificationSnoozeUntil } from "@/lib/notification-reminders";
import { NotificationIntelligenceControls } from "./notification-intelligence-controls";
...
export function NotificationSettingsCard({ data, setData, requestPermission, setToast }: Props) {
  ...
  const [snoozeUntil, setSnoozeUntil] = useState<number | null>(() => typeof window === "undefined" ? null : parseNotificationSnoozeUntil(localStorage.getItem(notificationSnoozeKey(localDateKey()))));
  ...
  function snoozeAllReminders() {
    if (typeof window === "undefined") return;
    const until = Date.now() + settings.snoozeMinutes * 60_000;
    localStorage.setItem(notificationSnoozeKey(localDateKey()), String(until));
    setSnoozeUntil(until);
    setToast(s("Reminders are snoozed for {minutes} minutes", { minutes: settings.snoozeMinutes }));
  }
  function resumeAllReminders() {
    if (typeof window === "undefined") return;
    localStorage.removeItem(notificationSnoozeKey(localDateKey()));
    setSnoozeUntil(null);
    setToast(s("Reminder snooze was cleared"));
  }
  ...
  return (
    <section id="settings-notifications" className="col-span-full scroll-mt-24 overflow-hidden dashboard-card rounded-[var(--card-radius)] border border-[var(--dashboard-border)] shadow-[0_5px_16px_rgba(0,0,0,.03)]">
      ...
      <fieldset disabled={!canEdit} className="grid gap-4 p-4 disabled:opacity-70 sm:p-5">
        ...
        <NotificationIntelligenceControls settings={settings} disabled={!canEdit} onUpdate={update} />
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-2)] p-4">
          <label className="grid max-w-xs gap-2 text-[10px] font-bold text-[var(--text-muted)]">{s("Default snooze duration")}<div className="flex items-center gap-2"><NumberField disabled={!canEdit} className="h-10" value={settings.snoozeMinutes} min={5} max={240} onValueChange={(value) => update("snoozeMinutes", Math.min(240, Math.max(5, value)))} /><span>{s("Minutes")}</span></div></label>
        </div>
      </fieldset>
      <div data-reminder-snooze className="flex flex-wrap items-center gap-2 border-t border-[var(--border)] p-4 sm:px-5"><Button type="button" variant="outline" onClick={() => void ensurePermission()} disabled={!canEdit}><BellRing /> {s("Request notification permission")}</Button><Button type="button" variant="secondary" onClick={() => void sendTestNotification()} disabled={permission === "unsupported"}><Send /> {s("Send test notification")}</Button><Button type="button" variant="secondary" onClick={snoozeAllReminders} disabled={!data.settings.notificationSettings.enabled}><PauseCircle /> {s("Snooze reminders")}</Button><Button type="button" variant="outline" onClick={resumeAllReminders} disabled={!snoozeUntil}><PlayCircle /> {s("Resume reminders")}</Button><Button type="button" variant="ghost" onClick={snoozeBreakReminderToday} disabled={!data.settings.notificationSettings.breakReminder.enabled}><TimerOff /> {s("Do not remind me today")}</Button></div>
