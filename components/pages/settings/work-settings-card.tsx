...
import type { AppData, Mode, WorkTimingMode } from "@/lib/types";
  ...
  return (
    <section id="settings-work-schedule" className="col-span-full scroll-mt-24 overflow-hidden dashboard-card rounded-[var(--card-radius)] border border-[var(--dashboard-border)] shadow-[0_5px_16px_rgba(0,0,0,.03)] max-[620px]:col-auto">
      ...
      <fieldset disabled={!canEdit} className="grid gap-5 p-4 disabled:opacity-70 sm:p-5">
        <div className="grid grid-cols-4 gap-[14px] max-[1100px]:grid-cols-2 max-[620px]:grid-cols-1">
          ...
          <label>{s("Work timing")}<Select value={settings.workTimingMode} onValueChange={(mode) => setSetting("workTimingMode", mode as WorkTimingMode)}><SelectTrigger data-work-timing-mode><SelectValue /></SelectTrigger><SelectContent><SelectItem data-work-timing-option="scheduled" value="scheduled">{s("Fixed schedule")}</SelectItem><SelectItem data-work-timing-option="flexible" value="flexible">{s("Flexible schedule")}</SelectItem></SelectContent></Select><small className="mt-1 block text-[9px] leading-4 text-[var(--text-muted)]">{settings.workTimingMode === "flexible" ? s("Start and stop whenever you need; daily targets stay independent from fixed clock times.") : s("Start and end times define the target for each enabled day.")}</small></label>
