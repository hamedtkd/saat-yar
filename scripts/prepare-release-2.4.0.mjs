...
const expected = manifest.verifiedMainMergeCommitPrefix;
const head = execFileSync("git", ["rev-parse", "--short=7", "HEAD"], { cwd: root, encoding: "utf8" }).trim();
const branch = execFileSync("git", ["branch", "--show-current"], { cwd: root, encoding: "utf8" }).trim();

if (branch !== "dev") {
  throw new Error(`Saatyar 2.4.0 finalization must be prepared on dev; current branch is ${branch || "detached HEAD"}.`);
}
if (head !== expected) {
  throw new Error(`Saatyar 2.4.0 Phase 180 baseline mismatch: expected ${expected}, current HEAD ${head}.`);
}

console.log(`Saatyar 2.4.0 Phase 180 baseline verified on dev: ${head}`);
