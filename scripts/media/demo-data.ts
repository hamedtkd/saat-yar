...
function record(date: string, start: string, end: string, note: string, overrides: Partial<WorkRecord> = {}): WorkRecord {
  return {
    date,
    start,
    end,
...
export function createMediaDemoData(anchorIso = "2026-08-07T10:30:00+03:30"): AppData {
  ...
  const patterns = [
    ["07:52", "16:40"], ["08:04", "17:05"], ["07:45", "16:18"], ["08:10", "16:55"],
    ["07:58", "16:31"], ["07:50", "17:14"], ["08:02", "16:36"],
  ] as const;
  for (let offset = -20; offset <= 0; offset += 1) {
    ...
    records[key] = record(key, start, end, offset === 0 ? "مرور برنامه هفته و تحویل نسخه طراحی نهایی" : "کارهای برنامه‌ریزی‌شده روزانه");
  }
  const today = dateKey(anchor);
  records[today] = record(today, "07:55", "16:48", "مرور نسخه نهایی داشبورد و آماده‌سازی انتشار", {
    breaks: [{ id: "break-demo", start: "10:25", end: "10:35", title: "استراحت کوتاه", paid: true }],
  });
  ...
  data.timeEntries = [
    { id: "time-1", clientId: "client-1", projectId: "project-1", task: "UI polish", startedAt: "2026-08-06T05:00:00.000Z", endedAt: "2026-08-06T08:30:00.000Z", note: "صفحه امروز", billable: true, effectiveRate: 850_000 },
    { id: "time-2", clientId: "client-2", projectId: "project-2", task: "Charts", startedAt: "2026-08-05T06:00:00.000Z", endedAt: "2026-08-05T09:10:00.000Z", note: "گزارش‌ها", billable: true, effectiveRate: 1_050_000 },
  ];
  data.expenses = [{ id: "expense-1", projectId: "project-1", clientId: "client-1", title: "ابزار طراحی", amount: 1_600_000, date: dateKey(shiftDate(anchor, -5)), category: "software", createdAt: dateKey(shiftDate(anchor, -5)) + "T08:00:00.000Z" }];
  data.invoices = [{
    id: "invoice-1", number: "SY-1405-021", clientId: "client-1", projectId: "project-1", issuedAt: dateKey(shiftDate(anchor, -4)), dueAt: dateKey(shiftDate(anchor, 10)), status: "sent",
    lines: [{ id: "line-1", description: "طراحی و توسعه داشبورد", quantity: 1, unitPrice: 32_000_000 }], discount: 0, taxPercent: 0, note: "مرحله دوم پروژه", createdAt: dateKey(shiftDate(anchor, -4)) + "T08:00:00.000Z",
  }];
  return data;
}
