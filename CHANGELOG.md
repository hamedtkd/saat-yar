...
- Phase 199 R11: Employee browser-contract hotfix restores the live activity title marker on the compact running panel after the R5/R6 visual refactor. The title and live timer were already rendered correctly, but the built browser smoke waited for `[data-active-activity-title]`; the semantic marker is restored without changing UI, timer behavior, AppData v21, dependencies, or lockfile.
- Phase 199 R10: Gate hotfix narrows normalized `WorkProject.status` back to the `active | archived` contract so TypeScript can prove AppData v21 normalization. No product behavior, schema, dependency, lockfile, GitHub-star, timer, or project-isolation behavior changes. The Employee browser isolation failure observed after the R9 typecheck failure came from reusing the previously built static export; browser smokes must run only after a fresh successful `check:quality` build.
- Phase 199 R8: Footer compile hotfix replaces the unsupported Lucide `Github` brand import with a source-owned accessible GitHub mark SVG while keeping the existing GitHub star CTA, About/Help/Privacy/Terms links, responsive footer layout, AppData v21, dependencies and lockfile unchanged.
- Phase 199 R7: Employee Today polish fixes the approved running-activity alignment without changing the FlipClock itself, makes the recent activity history internally scrollable instead of truncating completed segments, and adds direct row-level editing for clock-in, clock-out, lunch, and breaks. Help-page alignment, the responsive app header, and the footer are also polished: About/Help/Privacy/Terms are reachable, mobile Profile becomes avatar-only, Workspace switching is compact, the logo is restored to a stronger size, and a GitHub star CTA links to the open-source repository. AppData remains v21 with no dependency or lockfile change.
- Phase 199 R6: Final running-bar polish keeps the approved compact Employee/Hybrid activity shell while moving the live-status pill to logical inline-start (right in Persian RTL, left in English LTR). The live FlipClock keeps its approved size and styling, and the Stop action now owns a separate logical-edge action column with a subtle divider instead of sharing the timer surface. Activity type, work item and project remain read-only display fields; timer behavior, AppData v21 and dependencies are unchanged.
- Phase 199 R5: Active Employee/Hybrid activity returns to the same compact form-sized shell used before tracking starts. Activity type, work item and project become read-only display fields while the existing compact live FlipClock and Stop action share the final action area, eliminating the oversized running panel without changing timer behavior. Duration editing now keeps a fixed visual order of Hours on the left and Minutes on the right in both RTL and LTR while each field preserves locale-aware label direction and LTR numeric input. AppData remains v21 with no dependency or lockfile change.
- Phase 199 R4: Activity live-state visual polish makes the Employee/Hybrid running surface direction-safe and closer to the approved compact reference. The duration editor keeps Hours/Minutes in logical Locale order with centered numeric fields and accessible steppers; the live FlipClock uses better-proportioned activity digits, aligned separators and unit labels, while the running panel uses neutral themed surfaces plus a subtle pulse instead of a large green banner. Activity category/recent-history cards are also tightened. AppData remains v21 with no dependency or lockfile change.
- Phase 199 R3: Employee/Hybrid activity tracking now uses a smaller boxed FlipClock for the live activity timer, replaces the full success-green running banner with a neutral accent-led live surface, aligns optional Work Item/Project labels inline, and adds completed-activity edit/delete actions. Editing changes only time spent (hours/minutes) while preserving the original start boundary and metadata; active segments are protected from history edits/deletion. AppData remains v21 with no dependency or lockfile change.
- Phase 199 R2: Employee/Hybrid activity context now owns a separate `workProjects` collection inside development AppData v21, so Employee Today no longer leaks Freelancer client projects. Work projects can be created inline from the activity form, Hybrid may optionally reference either work or freelance project context, the activity-title/project/type controls share one aligned field hierarchy, and browser coverage now verifies project isolation + inline creation + persistence. No dependency or package-lock change.
