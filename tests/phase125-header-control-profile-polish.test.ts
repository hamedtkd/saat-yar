...
test("profile trigger reads as a real profile with a round avatar and local identity", async () => {
  ...
  assert.match(source, /پروفایل محلی/);
  assert.match(source, /aria-label=\{`پروفایل \$\{displayName\}`\}/);
...
test("sidebar brand is a guarded shortcut back to today", async () => {
  ...
  assert.match(source, /<GuardedLink[\s\S]*href="\/today"[\s\S]*aria-label="رفتن به صفحه امروز"[\s\S]*<Brand/);
});
